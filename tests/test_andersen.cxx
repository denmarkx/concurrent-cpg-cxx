...
TEST_CASE("Andersen[FieldSensitivity_Nested_Global]") {
    ...
    const Value *s1 = findInstr(F, "loadS1");
    const Value *s2 = findInstr(F, "loadS2");
...
TEST_CASE("Andersen[HeapCopy]") {
    ...
    const Value *loadF1 = findInstr(F1, "loadF1"); 
    const Value *loadF2 = findInstr(F2, "loadF2"); 
    PtsSetType set;

    anders->connectContexts(f, F1);
    bool success = anders->addConstraint(AndersConstraint::COPY, F1A, main, 1, 1);
    REQUIRE(success);
    anders->resolveConstraints();

    PtsSetType ptsSetA;
    anders->getPointsToSet(loadF1, ptsSetA, 2u);
    CHECK(ptsContains(ptsSetA, local));
    CHECK(ptsSetA.size() == 3); // local, main_fn, f1a_fn

    PtsSetType ptsSetB;
    anders->getPointsToSet(loadF2, ptsSetB, 3u);
    CHECK(ptsContains(ptsSetB, local));
    CHECK(ptsSetB.size() == 3); // local, main_fn, f1a_fn
}
...
TEST_CASE("Andersen[FieldSensitivity_Constant_Global_Nested]") {
    ...
    CHECK(ptsContains(ptsSetA, f4));
    CHECK(ptsContains(ptsSetA, f5));
    CHECK(ptsSetA.size() == 3); // @in1, f5, f4
